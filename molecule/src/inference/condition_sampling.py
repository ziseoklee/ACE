...
from pipelines.expert_utils import mol_atom_type_indices
from postprocessing import MoleculeBuilder
from sampling.moe_layout import DynamicMoELayout
from sampling.path_factory import build_padded_expert_path
...
def build_condition_probability_path(
    ...
    fragment_atom_types = mol_atom_type_indices(condition.fragment, layout=layout, device=device)
    ...
    for component_runtime in runtime.components:
        ...
        component_runtime.pipeline.prepare_data(
        ...
        component_sbdd_expert = component_runtime.pipeline.sbdd_expert(component_runtime)
        if component_sbdd_expert is not None:
            if sbdd_expert is not None:
                raise ValueError("Multiple SBDD components are not supported by the current sampler initialization.")
            sbdd_expert = component_sbdd_expert
