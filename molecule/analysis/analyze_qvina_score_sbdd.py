...
import os
import ast
from utils.utils import sandbox_import_dir, get_project_root
...
def main(argv):
    ...
    analyze_df = pd.read_csv(os.path.join(analysis_res_dir, 'crossdock', 'sbdd_len_dict.csv'))
    ...
    df = pd.DataFrame(task_list, index=task_list)
