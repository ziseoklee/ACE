...
def simulate_hcg_generalized(
    ...
    proj_list: list,         # list of projection functions
    inv_proj_list: list,     # list of inverse projection functions
    sigma_fn: callable,
    ...
    for i in range(n_steps):
        ...
        v_star_t = v_star(x, t)
        s_star_t = sum([gamma_list[i] * inv_proj_list[i](s_fn_list[i](proj_list[i](x), t)) for i in range(len(gamma_list))])
        div_v_star_t = divergence(v_star, x, t)
        other_terms = sum([gamma_list[i]*(-divergence(lambda _x, _t: inv_proj_list[i](v_fn_list[i](proj_list[i](_x), _t)), x, t) + torch.sum((v_star_t - inv_proj_list[i](v_fn_list[i](proj_list[i](x), t))) * inv_proj_list[i](s_fn_list[i](proj_list[i](x), t)), dim=1, keepdim=True)) for i in range(len(gamma_list) )])
