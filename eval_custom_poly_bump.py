...
top_experiment_id = "#Final_Evaluation_Results#_fixed_(Bump case)"  # Use a fixed ID for reproducibility
...
bs = 10000; n_steps=1000; ess_threshold = 0.7; Bump = 100
...
subexperiment_id = f"{names}" + f"_Best_ESS_{ess_threshold}_Bump{Bump}"
...
for seed in seeds:
    ...
    # plot_diagnostics(samples, logw_final, logw_history, save_name=f"{experiment_id}/alpha={names}_NR_seed{seed}")
    # plot_path_trajectories(sample_history, n_frame=6, resample_history=None, experiment_id=experiment_id, name=f"alpha={names}_NR_seed{seed}", deg=-50)
    ...
    samples = samples.cpu().numpy()
    plot_diagnostics(samples, logw_final, logw_history, save_name=f"{experiment_id}/alpha={names}_ACE_seed{seed}")
    plot_path_trajectories(sample_history, n_frame=6, resample_history=None, experiment_id=experiment_id, name=f"alpha={names}_ACE_seed{seed}", deg=-50)
    plt.close(); clear_output()
...
df.to_csv(f"{experiment_id}/experiment_results_numseeds{len(seeds)}_bs{bs}_n_steps{n_steps}_ess{ess_threshold}.csv", index=False)
print(df)