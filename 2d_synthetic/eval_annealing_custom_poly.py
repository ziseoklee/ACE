...
subexperiment_id = f"{names}" + f"_Best_ESS_{ess_threshold}_bs{bs}_n_steps{n_steps}_annealing"
...
for g in gs:
    ...
    df.to_csv(f"{experiment_id}/experiment_results_numseeds{len(seeds)}_bs{bs}_n_steps{n_steps}_ess{ess_threshold}_g{g}.csv", index=False)
    print(df)